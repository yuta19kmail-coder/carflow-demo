...
function renderLanding() {
  ...
  // v1.8.105/106: 金バーは「確定（fixed）が目標到達」した時のみ。ピッタリ達成でも金（>=）
  const fixedOverCount = L.fixed.count > 0 && L.fixed.count >= totalGoalCount;
  ...
  const fixedOverSales = fxSales > 0 && fxSales >= totalGoalSales;
