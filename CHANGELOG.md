...
- **車台数を増加**：22台 → 33台（その他3／仕入6／再生7／展示9／納車準備5／納車済3）。
- **大タスクメモ**：設定 `appTaskMemoConfig` を追加（再生 webUP=日付「掲載予定日」／見積=自由メモ、納車 登録内容=日付「登録予定日」／納車整備=時刻「入庫予定」）。複数の車に `taskMemos` を実値で投入。
- **緑付箋の自動付与**：日付メモ（webUP掲載予定日／登録予定日）から `_makeAutoStickies()` で 🤖自動 の緑付箋を生成して boardNotes に投入（`autoSource.type='taskMemo'`、status未設定なので起動時cleanupで消えない）。
- **納車カレンダーの日付メモ**：納車準備車に「登録予定日(日付)」メモ → カレンダーの車両バーに表示。
- **選択制大タスク**：カスタムタスク「下取り査定」(`c_appraisal`) を `appCustomTasks`＋`appTaskOptional` で用意し、再生車の一部だけ `selectedTasks` でON。
- **小タスクバリエーション**：納車準備・納車済み車の「登録内容(d_register)」に、選択式（登録パターン）＋あり/なし/該当なし(tri)6項目をランダム投入 → 登録内容バー＆多様な入力タイプを実演（`_makeRegisterData()`）。
- **操作ログ**：メモ更新／選択タスク追加／登録内容設定／バックオフィス完了 などの文言を追加。

`js/demo-sample-data.js`(?v=3)。バージョン `v2.5.31-demo`。
**※デモ専用**（本体は実データで運用するため、この刷新はデモのみ）。

---

### 2026-05-21：お知らせに過去アップデートまとめ＋seed（既読扱い）機構を反映（v2.5.30-demo）

本体 v2.7.0 で追加した「過去のメインどころ」お知らせと seed 機構をデモへ反映。

- **履歴お知らせ7件を追加**（古い順）：🔄リアルタイム同期 / 💴税モード / 🔔LINE通知＆2軸期日 / 📊ダッシュボード刷新 / 🗂バックオフィス / 📝タスク個別メモ / 📦タスクパターン。すべて `seed: true`（公開時点で既読扱い・ポップアップ無し）。
- **`_isAncRead()`** を追加し、seed と保存済み既読の両方を「既読」と判定（`announceUnreadCount`／`renderAnnounce`／`_unreadAncSorted` の3箇所を差し替え）。
- **デモ固有**：直近3件（4テーマ／メモ一覧／操作ログ）は **seed を付けず**、ポップアップのデモンストレーション用に残す（ログイン後に古い順で3件ポップアップ）。本体は全件 seed（全員既読扱い）なのでこの点だけ意図的に差をつけている。既読保存はデモ＝localStorage のまま。

`js/announcements.js`(?v=6)。バージョン `v2.5.30-demo`。
